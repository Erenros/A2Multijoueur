...
bool GCSolutionGenerator::CommandCreate(string exe, vector<string>& args)
{
	if ( args.size()<3 )
	{
		cerr << RED << "Incorrect number of arguments. Use -help to see available commands." << RESET << endl;
		return false;
	}
	...
	// .bat
	ofstream bat(folder+"bin\\make.bat");
	bat << "SolutionGenerator.exe -make ..\\";
	bat.close();

	// .exe
	ifstream exeSrc(exe, ios_base::binary);
	exeSrc.seekg(0, ios_base::_Seekdir::_Seekend);
	std::streampos size = exeSrc.tellg();
	exeSrc.seekg(0);
	char* src = new char[size];
	exeSrc.read(src, size);
	exeSrc.close();
	ofstream exeTrg(folder+"bin\\SolutionGenerator.exe", ios_base::binary|ios_base::trunc);
	exeTrg.write(src, size);
	exeTrg.close();
	delete [] src;
	return true;
}
...
bool GCSolutionGenerator::CommandMake(vector<string>& args)
{
	...
	if ( params.size()>=2 )
		projects = SplitString(params[1], ',');

	// Generate
	string folder = ToFolder(params[0]);
	return GenerateVisualSolution(folder, projects, isForceRemove);
}
...
void GCSolutionGenerator::ModifyJsonSolution(string folder, string projectNameToAdd)
{
	...
	if ( solutionPath=="" )
	{
		cerr << RED << "Solution not found." << RESET << endl;
		return;
	}
...
bool GCSolutionGenerator::GenerateVisualSolution(string folder, unordered_set<string>& projects, bool deleteFolder)
{
	if ( folder=="" || fs::exists(folder)==false )
	{
		cerr << RED << "Folder not found." << RESET << endl;
		return false;
	}
	...
	if ( solutionPath=="" )
	{
		cerr << RED << "No .sol file found." << RESET << endl;
		return false;
	}
	...
	if ( deleteFolder && projects.size()==0 )
		DeleteFolderSafe(m_ideFolder);

	// Generate solution
	if ( GenerateSln()==false )
		return false;
	...
	{
		if ( projects.size() && projects.find(GetStr(project, "name"))==projects.end() )
			continue;
	...
	if ( count==0 )
	{
		cerr << RED << "No projects found." << RESET << endl;
		return false;
	}
...
bool GCSolutionGenerator::GenerateSln()
{
	...
	std::cout << "Creating solution file: " << filePath << endl;

	ofstream outputFile(filePath);
	if ( outputFile.is_open()==false )
	{
		cerr << RED << "Failed to create file." << filePath << RESET << endl;
		return false;
	}
...
void GCSolutionGenerator::FillDataWithSolAndPrj()
{
	...
	std::cout << "Reading data from " << m_solutionPath << endl;
	if ( inputPath.is_open()==false )
	{
		cerr << RED << "Could not open the file " << m_solutionPath << RESET << endl;
		return;
	}
...
void GCSolutionGenerator::WriteJsonFile(const string& fileName, const ordered_json& data)
{
	...
	{
		cerr << RED << "Unable to open file for writing: " << fileName << RESET << endl;
	}
}
